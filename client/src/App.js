...
import NGToken from "./contracts/NGToken.json";
...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.NGToken = new this.web3.eth.Contract(
        NGToken.abi,
        NGToken.networks[this.networkId] && NGToken.networks[this.networkId].address,
      );
  ...
  updateUserTokens = async () => {
    let userTokens = await this.NGToken.methods.balanceOf(this.accounts[0]).call();
    this.setState({userTokens: userTokens});
  }

  listenToTokenTransfer = async () => {
    this.NGToken.events.Transfer({to: this.accounts[0]}).on("data", this.updateUserTokens);
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>New generation Token for Sustaiable energy!</h1>
